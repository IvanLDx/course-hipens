...
const PaymentStatus = ({ order }) => {
	return (
		<View style={styles.card}>
			...
			<View style={[styles.statusBox, order.isPaid && styles.successStatus]}>
				{order.isPaid && <Message variant="success">`Successfully paid on ${new Date(order.paidAt).toLocaleString()}`</Message>}
			</View>
			<View>{order.isDelivered && <Message variant="success">`Successfully delivered on ${new Date(order.deliveredAt).toLocaleString()}`</Message>}</View>
		</View>
	);
};
