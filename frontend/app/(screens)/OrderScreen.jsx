...
export default function OrderScreen() {
	...
	// --- RTK Query Hooks ---
	const { data: order, isLoading: isLoadingOrder, error: orderError, refetch: refetchOrder } = useGetOrderDetailsQuery(orderId);

	const [createPayPalOrder, { isLoading: isCreatingPayPalOrder, error: createPayPalError }] = useCreatePaypalOrderMutation();
	const [capturePayPalOrder, { isLoading: isCapturingPayPalOrder, error: capturePayPalError }] = useCapturePaypalOrderMutation();
	const [payOrder, { isLoading: isPayingOrder, error: payOrderError }] = usePayOrderMutation();
	const [deliverOrder, { isLoading: isLoadingDeliver, error: deliverError }] = useDeliverOrderMutation();

	const { data: paypalConfig, isLoading: isLoadingPayPalConfig, error: paypalConfigError } = useGetPaypalClientIdQuery();
	...
	useEffect(() => {
		...
		if (deliverError) {
			Toast.show({
				...
				text2: deliverError?.data?.message || 'Failed to mark order as delivered.',
				position: 'top',
				visibilityTime: 7000
			});
		}
  ...
  `,
	...
	const onMessage = useCallback(
			...
			try {
				...
				switch (data.type) {
					case 'PAYPAL_APPROVED':
						...
						try {
							const captureResponse = await capturePayPalOrder(data.orderID).unwrap();
							...
							if (captureResponse.status === 'COMPLETED') {
								const paymentResult = {
									...
									email_address: captureResponse.payer?.email_address || 'not.provided@example.com'
								};
								...
								Toast.show({
									...
									text2: `PayPal payment capture failed. Status: ${captureResponse.status || 'Unknown'}`,
									position: 'top',
									visibilityTime: 7000
								});
							}
						} catch (err) {
							const errorMessage = err?.data?.details || err?.data?.message || err.message || 'Unknown error occurred';
							Toast.show({
								type: 'error',
								text1: 'Payment Error',
					...
					case 'REQUEST_PAYPAL_ORDER_ID':
						if (!order || !order.totalPrice) {
							const errorMessage = 'Order details or total price missing for PayPal order creation.';
							Toast.show({
								type: 'error',
								text1: 'Payment Error',
								text2: errorMessage,
						...
						try {
							...
							if (res.id) {
								...
								const errorMessage = 'No PayPal ID received from backend create order response.';
								Toast.show({
									type: 'error',
									text1: 'Payment Error',
									text2: errorMessage,
							...
							const errorMessage = `Failed to create PayPal order on backend: ${err?.data?.message || err.message || err.toString()}`;
							Toast.show({
								type: 'error',
								text1: 'Payment Error',
								text2: errorMessage,
					...
					default:
						Toast.show({
							type: 'error',
							text1: 'Error',
							text2: 'An unexpected error occurred with the PayPal payment message.',
							position: 'top',
							visibilityTime: 7000
						});
						setShowWebViewModal(false);
		...
		[order, orderId, createPayPalOrder, capturePayPalOrder, payOrder, refetchOrder]
	);
	...
	if (orderError) {
		return (
			<View style={styles.center}>
				<Text style={styles.errorText}>Error: {orderError?.data?.message || 'Failed to load order details'}</Text>
			</View>
		);
	}
	...
	if (!order) {
		return (
			<View style={styles.center}>
				<Text style={styles.errorText}>Order not found or no data available.</Text>
			</View>
		);
	}
